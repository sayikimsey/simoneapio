...
//   }
// };
const nodemailer_1 = __importDefault(require("nodemailer")); // Importiert die Nodemailer-Bibliothek zum Senden von E-Mails
// -------------------------------------------------------------------
...
const sendPasswordResetEmail = async (to, token) => {
    ...
    const resetLink = `${process.env.FRONTEND_URL || "http://localhost:3000" // Fallback auf localhost für Entwicklung
    ...
    try {
        ...
        if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
            console.error("🔥 FATAL: E-Mail-Anmeldedaten (EMAIL_USER, EMAIL_PASS) sind in der .env-Datei nicht konfiguriert.");
        ...
        const info = await transporter.sendMail(mailOptions);
        console.log(`✅ Passwort-Reset-E-Mail an ${to} gesendet: ${info.messageId}`);
    }
    catch (error) {
        // Fehler beim Senden der E-Mail abfangen und protokollieren.
        console.error(`🔥 Fehler beim Senden der Passwort-Reset-E-Mail an ${to}:`, error);
