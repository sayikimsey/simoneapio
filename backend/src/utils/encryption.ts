...
import crypto from "crypto"; // Importiert das Node.js 'crypto'-Modul für kryptografische Operationen
...
if (!ENCRYPTION_KEY_HEX || ENCRYPTION_KEY_HEX.length !== 64) {
  console.error(
    "🔥 FATAL: MFA_ENCRYPTION_KEY ist nicht in .env definiert oder ist kein 64-stelliger Hex-String (32 Bytes)."
  );
...
export const encryptMfaSecret = (plainSecret: string): string | null => {
  ...
  if (!ENCRYPTION_KEY_HEX || KEY.length !== 32) {
    console.error(
      "🔥 MFA-Verschlüsselung fehlgeschlagen: Verschlüsselungsschlüssel ist ungültig oder fehlt."
    );
...
export const decryptMfaSecret = (encryptedValue: string): string | null => {
  ...
  if (!ENCRYPTION_KEY_HEX || KEY.length !== 32) {
    console.error(
      "🔥 MFA-Entschlüsselung fehlgeschlagen: Verschlüsselungsschlüssel ist ungültig oder fehlt."
    );
    return null; // Oder einen Fehler werfen
  }
  try {
